...
var dataUrl, collegetag, college_tag = 233,
    ...
    targetid, dataurl, like, likeNum, qformdata, aformdata, orgPic, thumbPic, action = 'http://wx.yyeke.com/XZBBM/uploadfile/';
var nick_name = [];
var tag = new Array();
...
document.querySelector('.sendit').onclick = function() {
    ...
    if (title == '') {
        ...
        function upload(q) {
            ...
            var url = 'http://wx.yyeke.com/XZBBM/bbm/discuss?type=question';
            ...
            xhr.onreadystatechange = function() {
                if (xhr.readyState == 4 && xhr.status == 200) {
                    ...
                    headpageshow();
                    questionshow('http://wx.yyeke.com/XZBBM/bbm/index');
                }
            }
...
document.querySelector('.sendanswer').onclick = function() {
    ...
    if (content == '') {
        ...
        function upload(q) {
            var xhr = new XMLHttpRequest();
            var url = 'http://wx.yyeke.com/XZBBM/bbm/discuss';
            xhr.open('POST', url, true);
            xhr.setRequestHeader("Content-type", "application/x-www-form-urlencoded");
            xhr.onreadystatechange = function() {
                if (xhr.readyState == 4 && xhr.status == 200) {
...
upload.onchange = function() {
    ...
    uploadBtn.style.backgroundImage = "url(imgs/loading.gif)";
    var xhr = new XMLHttpRequest();
    var url = 'http://wx.yyeke.com/XZBBM/bbm/pic_upload.do';
    xhr.open('POST', url, true);
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && xhr.status == 200) {
            console.log('Got a POST request');
...
upphotoload.onchange = function() {
    ...
    upphoto.style.backgroundImage = "url(imgs/loading.gif)";
    var xhr = new XMLHttpRequest();
    var url = 'http://wx.yyeke.com/XZBBM/bbm/pic_upload.do';
    xhr.open('POST', url, true);
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            upphoto.style.backgroundImage = "url(imgs/camera.png)";
        }
    }
    xhr.send(formData);
}
...
qdeleteimg.onclick = function() {
    ...
    imageshow.src = '';
    uploadBtn.style.backgroundImage = "url(imgs/camera.png)";
}
...
function myquestionshow() {
    var xhr = new XMLHttpRequest();
    xhr.open("GET", 'http://wx.yyeke.com/XZBBM/bbm/index', true);
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && xhr.status == 200) {
            arr = JSON.parse(xhr.responseText);
            var size = arr.data.size;
...
newq.onclick = function() {
    ...
    newq.style.backgroundImage = 'url(imgs/Chosebg.png)';
    hotq.style.backgroundImage = 'url(imgs/noChosebg.png)';
...
hotq.onclick = function() {
    ...
    hotq.style.backgroundImage = 'url(imgs/Chosebg.png)';
    newq.style.backgroundImage = 'url(imgs/noChosebg.png)';
...
myq.onclick = function() {
    ...
    myq.style.backgroundImage = 'url(imgs/Chosebg.png)';
    myanswer.style.backgroundImage = 'url(imgs/noChosebg.png)';
...
myanswer.onclick = function() {
    ...
    myanswer.style.backgroundImage = 'url(imgs/Chosebg.png)';
    myq.style.backgroundImage = 'url(imgs/noChosebg.png)';
...
function replyshow() {
    reply.innerHTML = '';
    var xhr = new XMLHttpRequest();
    var url1 = 'http://wx.yyeke.com/XZBBM/bbm/discuss' + '?id=' + targetid;
        ...
        if (xhr.readyState == 4 && xhr.status == 200) {
            ...
            for (var i = 0; i < size; i++) {
                ...
                n.innerHTML = '<div class="reply_avatar"><img class="myhead" src="' + head_url[i] + '"></div>' +
                    ...
                    '<img class="xueba" src="imgs/xueba.png">' +
                    '<div class="took notake" ></div>' +
                    '</div><div class="replyWords">' +
                    '<p>' + content[i] + '</p></div>' +
                    '<div class="reply_Img">' +
                    '<img class="qdetails_imgs" src="' + action + pic_name[i] + '"></div>' +
                    '<div class="reply_bottom">' +
                    '<p class="datetime">' + gettime(create_time[i]) + '</p>' +
                    '<p class="reply_college">' + tag[i] + '</p>' +
                    '<div class="reply_likeit like">' +
                    '<div class="likeIcon"></div>' +
                    '<p class="likeNum">' + like_count[i] + '</p></div></div></div>'

                if (pic_name[i] == 2333) {
                    var g4 = document.querySelectorAll(".qdetails_imgs");
                    g4[i].style.display = 'none';
                }
                var qdetails_imgs = document.querySelectorAll('.qdetails_imgs');
                ...
                if (arr.data.repliesList[i].accepted) {
                    took[i].className = 'take';
                }
                console.log(ansid[i]);
                took[i].onclick = function() {
                    if (arr.data.mine) {
                        if (this.className == 'notake') {
                            this.className = 'take';
                            var xhr = new XMLHttpRequest();
                            xhr.open('GET', 'http://wx.yyeke.com/XZBBM/bbm/discuss/operation?id=' + ansid[i] + '&type=accept', true);
                            xhr.onreadystatechange = function() {
                                if (xhr.readyState == 4 && xhr.status == 200) {
                            ...
                            this.className = 'notake';
                            var xhr = new XMLHttpRequest();
                            xhr.open('GET', 'http://wx.yyeke.com/XZBBM/bbm/discuss/operation?id=' + ansid[i] + '&type=not_accept', true);
                            xhr.onreadystatechange = function() {
                                if (xhr.readyState == 4 && xhr.status == 200) {
    ...
    for (let i = 0; i < a.length; i++) {
        a[i].addEventListener("click", function() {
            ...
            if (this.children[0].className == 'likeIcon') {
                ...
                var xhr = new XMLHttpRequest();
                xhr.open('GET', 'http://wx.yyeke.com/XZBBM/bbm/discuss/operation?id=' + likeid + '&type=like', true);
                xhr.onreadystatechange = function() {
                    if (xhr.readyState == 4 && xhr.status == 200) {
                ...
                this.children[1].innerHTML = parseInt(this.children[1].innerHTML) - 1;
                var xhr = new XMLHttpRequest();
                xhr.open('GET', 'http://wx.yyeke.com/XZBBM/bbm/discuss/operation?id=' + likeid + '&type=unlike', true);
                xhr.onreadystatechange = function() {
                    if (xhr.readyState == 4 && xhr.status == 200) {
...
function headpageshow() {
    headpage.style.backgroundImage = "url(imgs/onheadPage.png)";
    qPage.style.backgroundImage = "url(imgs/qPage.png)";
    sPage.style.backgroundImage = "url(imgs/sPage.png)";
    myPage.style.backgroundImage = "url(imgs/myPage.png)";
...
sPage.onclick = function() {
    headpage.style.backgroundImage = "url(imgs/headPage.png)";
    qPage.style.backgroundImage = "url(imgs/qPage.png)";
    sPage.style.backgroundImage = "url(imgs/onsPage.png)";
    myPage.style.backgroundImage = "url(imgs/myPage.png)";
    ...
    document.querySelector("#searchBox").onsearch = function() {
        ...
        var xhr = new XMLHttpRequest();
        if (college_tag == 233) {
            url2 = 'http://wx.yyeke.com/XZBBM/bbm/search?key= ' + key
        } else {
            url2 = 'http://wx.yyeke.com/XZBBM/bbm/search?key= ' + key + '&tag=' + college_tag
        }
...
qPage.onclick = function() {
    headpage.style.backgroundImage = "url(imgs/headPage.png)";
    qPage.style.backgroundImage = "url(imgs/onqPage.png)";
    sPage.style.backgroundImage = "url(imgs/sPage.png)";
    myPage.style.backgroundImage = "url(imgs/myPage.png)";
    contents.style.display = "none";
...
myPage.onclick = function() {
    ...
    qPage.style.backgroundImage = "url(imgs/qPage.png)";
    sPage.style.backgroundImage = "url(imgs/sPage.png)";
    myPage.style.backgroundImage = "url(imgs/onmyPage.png)";
    contents.style.display = "none";
    question_Page.style.display = "none";
    search_Page.style.display = "none";
...
function infoshow() {
    var xhr = new XMLHttpRequest();
    xhr.open("GET", 'http://wx.yyeke.com/XZBBM/bbm/user', true);
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && xhr.status == 200) {
            arr = JSON.parse(xhr.responseText);
            console.log(arr);
